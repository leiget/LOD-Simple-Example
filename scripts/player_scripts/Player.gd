...
#			SOUND
#############################
var player_moving_FWandBW = false
var player_moving_LeftAndRight = false
var step_interval_sec_base = 0.35
var step_interval_sec = step_interval_sec_base
var step_timer = 0.0
onready var step_01 = get_node("Step_01")
onready var step_02 = get_node("Step_02")
onready var step_03 = get_node("Step_03")
var steps_array = []
var current_soundstream = 0
...
func _ready():
	...
	steps_array.append(step_01)
	steps_array.append(step_02)
	steps_array.append(step_03)
...
func _physics_process(delta):
	...
	if(Input.is_action_pressed(String_Shift)):
		...
		# Say that the shift key is pressed.
		Pressed_Shift = true
	# Otherwise...
	else:
	...
	if(Pressed_LEFT and not Pressed_RIGHT):
		...
		player_moving_LeftAndRight = true
	#	RIGHT		#
	...
	#	BOTH		#
		...
		player_moving_LeftAndRight = false
	#	NEITHER		#
	...
	Debug_Label_String = str(get_floor_velocity())
	
	var snap = Vector3(0,0,0)
	
	if(Jump_Released == false):
		snap = Vector3(0,0,0)
	else:
		snap = Vector3(0 , (-Player_Height*0.5)-0.1, 0)
	...
	#move_and_slide_with_snap(FinalMoveVel, snap, FloorNormal, true, false, 4, MaxFloorAngleRad)
	...
	Debug_Label_String += "\nLOD Processing: " + str(get_tree().get_root().get_node("Main").LOD_Process_Active)
	Debug_Label_String += "\nNumber of LOD Objects: " + str(get_tree().get_root().get_node("Main").LOD_Number)
	Debug_Label_String += "\nOn-Screen Verts: " + str(Performance.get_monitor(Performance.RENDER_VERTICES_IN_FRAME))
	#Set the debug text.
	Debug_Label.set_text(Debug_Label_String)
